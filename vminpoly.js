...
(function() {
  var XMLHttpFactories, ajax, applyStyleTest, browserSupportsUnitsNatively, clearStyleTests, createXMLHTTPObject, getViewportSize, initLayoutEngine, testElementStyle, testVHSupport, testVMinSupport, testVWSupport, gbaValToString;
  ...
  gbaValToString = function(token) {
    var css = "";
    if (token && (token.name === "rgba" || token.name === "gba")) {
        css += token.name + "(";
        for (var i = 0, len = token.value.length; i < len; i++) {
            ...
            var v = token.value[i];
            for (var j = 0, jlen = v.value.length; j < jlen; j++) {
                var t = v.value[j];
  ...
  testVMinSupport = function(element, style_block) {
    ...
    one_vw = docElement.clientWidth / 2;
    one_vh = docElement.clientHeight / 2;
    actual_vmin = parseInt(Math.min(one_vw, one_vh), 10);
      ...
      generateRuleCode = function(rule) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          ...
          for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
            ...
            } else if (token.toSourceString) {
              ruleCss += token.toSourceString();
            } else {
              ruleCss += gbaValToString(token);
            }
          }
          ruleCss += ";";
        }
  ...
  var isSupportUnitsNatively = browserSupportsUnitsNatively();

  console.log('About to do the engine unless...', isSupportUnitsNatively);

  if (!isSupportUnitsNatively) {
    initLayoutEngine();
  }
